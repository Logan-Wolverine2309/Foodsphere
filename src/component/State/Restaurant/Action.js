...
export const createCategoryAction = ({ reqData, jwt, restaurantId }) => async (dispatch) => {
  dispatch({ type: CREATE_CATEGORY_REQUEST });
  try {
    const { data } = await api.post(`/api/admin/restaurants/${restaurantId}/category`, reqData, {
      headers: { Authorization: `Bearer ${jwt}` },
    });
...
export const getRestaurantCategory = ({ restaurantId, jwt }) => async (dispatch) => {
  ...
  try {
    const { data } = await api.get(`/api/admin/restaurants/${restaurantId}/category`, {
      headers: { Authorization: `Bearer ${jwt}` },
    });
